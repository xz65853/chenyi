...
if($do==""){
	...
	$info_num=mysql_query("SELECT * FROM `cs_zhengxing` as s where 1=1 $search");//当前频道条数
	...
	//查询
	$sql="SELECT * FROM `cs_info` as i,`cs_zhengxing` as s where s.infoid=i.id $search order by s.id desc  LIMIT $pageNum,$numPerPage";
	$db->query($sql);
	$list=$db->fetchAll();
	//echo $sql;
	//格式化输出数据
	...
	$smt->assign('title',"整形科档案");
	$smt->display('zhengxing_list.htm');
	exit;
...
if($do=="add"){
	...
	$sql="INSERT INTO `cs_zhengxing` (`infoid`,`sellid`,`intro`,`zdjg`,`zlff`,`zlcs`,`zzys`,`zxr`,`created_at`,`salesid_mr` )
	...
	if($db->query($sql)&&$db->query($sql2)){echo "{\"statusCode\":\"200\",\"message\":\"操作成功!\",\"navTabId\":\"\",\"callbackType\":\"forward\",	\"forwardUrl\":\"?action=zhengxing\"}";}
	else{echo "{\"statusCode\":\"300\",\"message\":\"操作错误!\",\"navTabId\":\"\",\"callbackType\":\"forward\",\"forwardUrl\":\"?action=zhengxing\"}";}
	exit;
}
//编辑	
if($do=="edit"){
	...
	//查询
	$sql="SELECT i.name,i.tel,i.xb,i.nl,s.id,s.infoid,s.zdjg,s.zlff,s.zlcs,s.zzys,s.zxr FROM `cs_zhengxing` as s,`cs_info` as i where s.id='$id' and s.infoid=i.id  LIMIT 1";
	$db->query($sql);
	$row=$db->fetchRow();
	...
	if($_SESSION[userid]==$row[salesid]&&$created_at2==$row[created_at]&&$_SESSION[roleid]!="1"){$smt->display('zhengxing_edit.htm');}
	elseif($_SESSION[roleid]=="1"){$smt->display('zhengxing_edit.htm');}
...
if($do=="updata"){
	...
	//sql
	$sql="UPDATE `cs_zhengxing` SET 
	`zdjg` = '$_POST[zdjg]',
	`zlff` = '$_POST[zlff]',
	`zlcs` = '$_POST[zlcs]',
	`zzys` = '$post_doctorid',
	`zxr` = '$_POST[zx_orgxh]',
	`intro` = '$_POST[intro]' WHERE `cs_zhengxing`.`id` ='$_POST[id]' LIMIT 1 ;";
...
if($do=="del"){
	If_rabc($action,$do); //检测权限
	$sql="delete from `cs_zhengxing` where `cs_zhengxing`.`id`=$id limit 1";
	if($db->query($sql)){echo "{\"statusCode\":\"200\",\"message\":\"操作成功!\",\"navTabId\":\"\",\"callbackType\":\"forward\"}";}
	else{echo "{\"statusCode\":\"300\",\"message\":\"操作错误!\",\"navTabId\":\"\",\"callbackType\":\"forward\"}";}		
	exit;
}
